...
import { CheckCircle2, ChevronDown, ChevronRight, Circle, Lock, Pencil, Plus, Scale } from '@lucide/vue';
...
const reconcileDifferenceMinor = computed(() =>
    actualBalanceMinor.value === null || !headerBalance.value ? null : actualBalanceMinor.value - headerBalance.value.clearedMinor,
);
...
async function finishReconciliation() {
    ...
    const adjustment = await repo.finishReconciliation(account.value.id, actualBalanceMinor.value);
    reconciling.value = false;
    actualBalanceMinor.value = null;
...
function payeeLabel(row: RegisterRow): string {
    ...
    return head.payee_id ? (payeesById.value.get(head.payee_id)?.name ?? '—') : '—';
}
...
function categoryLabel(row: RegisterRow): string {
    ...
    return row.transaction.category_id ? (categoriesById.value.get(row.transaction.category_id)?.name ?? '—') : '';
}

function categoryNameOf(transaction: Transaction): string {
    return transaction.category_id ? (categoriesById.value.get(transaction.category_id)?.name ?? '—') : 'Uncategorized';
}
    ...
    <div class="flex h-full flex-col">
        <div class="flex flex-wrap items-center gap-4 border-b px-4 py-3">
            ...
            <div class="ml-auto flex items-center gap-2">
                ...
                >
                    <Scale class="size-4" /> <span class="hidden sm:inline">Reconcile</span>
                </Button>
        ...
        <div v-if="reconciling && account" class="border-b bg-muted/40 px-4 py-3" data-testid="reconcile-banner">
            <div class="flex flex-wrap items-end gap-4">
                <div class="grid gap-1">
                    <label class="text-xs text-muted-foreground" for="actual-balance">Actual balance at the bank ({{ account.currency }})</label>
                    <div class="w-40">
                        <MoneyInput id="actual-balance" v-model="actualBalanceMinor" :currency="account.currency" />
                    </div>
                </div>
                <div v-if="reconcileDifferenceMinor !== null" class="pb-1 text-sm">
                    <span v-if="reconcileDifferenceMinor === 0" class="text-green-600">Matches the cleared balance.</span>
                    <span v-else class="text-amber-600 dark:text-amber-500" data-testid="reconcile-difference">
                        Difference: {{ formatMoney(reconcileDifferenceMinor, account.currency) }} — clear/adjust transactions below, or
                        finish to create an adjustment.
                    </span>
                </div>
                <div class="ml-auto flex gap-2 pb-1">
                    <Button variant="secondary" size="sm" @click="reconciling = false; actualBalanceMinor = null">Cancel</Button>
                    <Button size="sm" data-testid="reconcile-finish" :disabled="actualBalanceMinor === null" @click="finishReconciliation">
                        Finish reconciliation
                    </Button>
                </div>
            </div>
            <p class="mt-1 text-xs text-muted-foreground">
                Tip: use the cleared toggles while comparing against your bank statement. Finishing marks all cleared transactions as
                reconciled.
            </p>
        </div>

        <div v-if="reconcileMessage" class="border-b bg-green-500/10 px-4 py-2 text-sm text-green-700 dark:text-green-400">
            {{ reconcileMessage }}
        </div>
        ...
        <table v-else class="hidden w-full text-sm sm:table">
            ...
            <tbody data-testid="register-table">
                <template v-for="row in rows" :key="rowKey(row)">
                    <tr class="cursor-pointer border-b hover:bg-muted/50" @click="openEdit(row)">
                        <td class="px-4 py-2 whitespace-nowrap tabular-nums">{{ headOf(row).date }}</td>
                        <td v-if="accountId === null" class="px-2 py-2">{{ accountsById.get(headOf(row).account_id)?.name }}</td>
                        <td class="px-2 py-2">{{ payeeLabel(row) }}</td>
                        <td class="px-2 py-2 text-muted-foreground">
                            ...
                            <template v-else>{{ categoryLabel(row) }}</template>
                        </td>
                        <td class="max-w-48 truncate px-2 py-2 text-muted-foreground">{{ headOf(row).memo }}</td>
                        <td class="px-2 py-2 text-right tabular-nums">
                            {{ amountOf(row) < 0 ? formatMoney(-amountOf(row), currencyOf(row)) : '' }}
                        </td>
                        <td class="px-2 py-2 text-right tabular-nums">
                            {{ amountOf(row) > 0 ? formatMoney(amountOf(row), currencyOf(row)) : '' }}
                        </td>
                        <td class="px-4 py-2 text-center" @click.stop="toggleCleared(row)">
                            <Lock v-if="headOf(row).cleared === 'reconciled'" class="inline size-4 text-muted-foreground" />
                            <CheckCircle2
                    ...
                    <tr
                        v-for="member in row.kind === 'split' && expandedGroups.has(row.groupId) ? row.members : []"
                        :key="member.id"
                        class="border-b bg-muted/30 text-xs"
                    >
                        <td class="px-4 py-1.5" />
                        <td v-if="accountId === null" class="px-2 py-1.5" />
                        <td class="px-2 py-1.5" />
                        <td class="px-2 py-1.5 pl-7 text-muted-foreground">{{ categoryNameOf(member) }}</td>
                        <td class="px-2 py-1.5" />
                        <td class="px-2 py-1.5 text-right tabular-nums text-muted-foreground">
                            {{ member.amount < 0 ? formatMoney(-member.amount, currencyOf(row)) : '' }}
                        </td>
                        <td class="px-2 py-1.5 text-right tabular-nums text-muted-foreground">
                            {{ member.amount > 0 ? formatMoney(member.amount, currencyOf(row)) : '' }}
                        </td>
        ...
        <div v-if="rows.length" class="divide-y sm:hidden" data-testid="register-cards">
            <div v-for="row in rows" :key="rowKey(row)">
                <button type="button" class="flex w-full items-center gap-3 px-4 py-3 text-left" @click="openEdit(row)">
                    <span @click.stop="toggleCleared(row)">
                        <Lock v-if="headOf(row).cleared === 'reconciled'" class="size-5 text-muted-foreground" />
                        <CheckCircle2 v-else-if="headOf(row).cleared === 'cleared'" class="size-5 text-green-600" />
                        <Circle v-else class="size-5 text-muted-foreground" />
                    </span>
                    <span class="min-w-0 flex-1">
                        <span class="block truncate text-sm font-medium">{{ payeeLabel(row) }}</span>
                        <span class="block truncate text-xs text-muted-foreground">
                            {{ headOf(row).date }}
                            <template v-if="accountId === null"> · {{ accountsById.get(headOf(row).account_id)?.name }}</template>
                            <template v-if="categoryLabel(row)"> · {{ categoryLabel(row) }}</template>
                        </span>
                    </span>
                    <span class="text-sm font-medium tabular-nums" :class="amountOf(row) > 0 ? 'text-green-600' : ''">
